import { ref } from 'vue'
import type { ParticipantDraft, Session, SpinResult } from '../types'
...
export function useSession() {
  ...
  async function addDrafts(drafts: ParticipantDraft[]) {
    ...
    error.value = null
    await roster.addDrafts(drafts).catch((e: any) => { error.value = e.message })
  }
  ...
  async function updateParticipant(
    ...
    if (!session.value) return
    error.value = null
    await roster.updateParticipant(participantId, patch).catch((e: any) => { error.value = e.message })
  }
