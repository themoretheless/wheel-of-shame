...
use crate::models::{
    Action, ActionKind, Participant, SegmentVisual, Session, Snapshot, SpinResult,
    MAX_PARTICIPANT_WEIGHT, MIN_PARTICIPANT_WEIGHT,
};
use serde_json;
...
fn row_to_participant(row: &SqliteRow) -> Participant {
    ...
    let weight = row
        .try_get::<i64, _>("weight")
        .ok()
        .map(|w| w as u32)
        .or_else(|| {
            row.try_get::<f64, _>("weight")
                .ok()
                .map(|w| w.round() as u32)
        })
        .unwrap_or(MIN_PARTICIPANT_WEIGHT)
...
impl SqliteStore {
    ...
    async fn init_schema(&self) -> Result<(), AppError> {
        ...
        self.ensure_participant_column("weight", "INTEGER NOT NULL DEFAULT 1")
            .await?;
        self.ensure_participant_column("visual", "TEXT").await?;
        ...
        // Migration for existing DBs (additive columns/tables)
        let _ = sqlx::query("CREATE TABLE IF NOT EXISTS actions (id TEXT PRIMARY KEY, session_id TEXT NOT NULL, kind TEXT NOT NULL, payload TEXT, timestamp TEXT NOT NULL, actor TEXT)").execute(&self.pool).await;
        let _ =
            sqlx::query("CREATE INDEX IF NOT EXISTS idx_actions_session ON actions(session_id)")
                .execute(&self.pool)
                .await;
        let _ = sqlx::query("CREATE TABLE IF NOT EXISTS snapshots (id TEXT PRIMARY KEY, session_id TEXT NOT NULL, action_id TEXT NOT NULL, participants TEXT NOT NULL)").execute(&self.pool).await;
...
impl Store for SqliteStore {
    ...
    async fn update_participant_props(
        ...
        if let Some(w) = weight {
            sqlx::query("UPDATE participants SET weight = ? WHERE session_id = ? AND id = ?")
                .bind(
                    (w.round() as u32).clamp(MIN_PARTICIPANT_WEIGHT, MAX_PARTICIPANT_WEIGHT) as i64,
                )
                .bind(session_id)
                .bind(participant_id)
                .execute(&mut *tx)
        ...
        if let Some(v) = &visual {
            ...
            sqlx::query("UPDATE participants SET visual = ? WHERE session_id = ? AND id = ?")
                .bind(vjson)
                .bind(session_id)
                .bind(participant_id)
                .execute(&mut *tx)
    ...
    async fn append_action(&self, action: &Action) -> Result<(), AppError> {
        let payload = match &action.kind {
            crate::models::ActionKind::Add { name } => {
                serde_json::json!({"name": name}).to_string()
            }
            crate::models::ActionKind::Remove { id } => serde_json::json!({"id": id}).to_string(),
            crate::models::ActionKind::Spin { picked_id } => {
                serde_json::json!({"picked_id": picked_id}).to_string()
            }
            crate::models::ActionKind::Reset => "{}".to_string(),
            crate::models::ActionKind::UpdateWeight { id, weight } => {
                serde_json::json!({"id": id, "weight": weight}).to_string()
            }
            crate::models::ActionKind::UpdateVisual { id, visual } => {
                serde_json::json!({"id": id, "visual": visual}).to_string()
            }
            crate::models::ActionKind::Reorder { from, to } => {
                serde_json::json!({"from": from, "to": to}).to_string()
            }
            crate::models::ActionKind::SnapshotRestored { snapshot_id } => {
                serde_json::json!({"snapshot_id": snapshot_id}).to_string()
            }
        };
        sqlx::query(
        ...
        for row in rows {
            ...
            let kind = match kind_str.as_str() {
                ...
                "Spin" => ActionKind::Spin {
                    picked_id: payload["picked_id"].as_str().unwrap_or("").to_string(),
                },
                "Reset" => ActionKind::Reset,
                "UpdateWeight" => ActionKind::UpdateWeight {
                    id: payload["id"].as_str().unwrap_or("").to_string(),
                    weight: payload["weight"].as_f64().unwrap_or(1.0) as f32,
                },
                "UpdateVisual" => ActionKind::UpdateVisual {
                    id: payload["id"].as_str().unwrap_or("").to_string(),
                    visual: serde_json::from_value(payload["visual"].clone()).unwrap_or_default(),
                },
                "Reorder" => ActionKind::Reorder {
                    from: payload["from"].as_u64().unwrap_or(0) as usize,
                    to: payload["to"].as_u64().unwrap_or(0) as usize,
                },
                "SnapshotRestored" => ActionKind::SnapshotRestored {
                    snapshot_id: payload["snapshot_id"].as_str().unwrap_or("").to_string(),
                },
